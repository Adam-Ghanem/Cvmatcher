...
The initial CI configuration was validated locally. A later remote run identified pnpm 11's ignored-build policy for the existing locked `unrs-resolver` postinstall. Its source was reviewed and only that one package was added to the workspace policy. The first remediation used the obsolete `onlyBuiltDependencies` key, which pnpm 11 ignores; it was immediately corrected to the supported `allowBuilds: { unrs-resolver: true }` form. The corrected remediation is validated locally with a frozen install and requires remote workflow confirmation after publication.
...
| `pnpm-workspace.yaml` | Uses pnpm 11’s `allowBuilds` map to approve only the reviewed existing `unrs-resolver` postinstall required for a reproducible frozen install. |
